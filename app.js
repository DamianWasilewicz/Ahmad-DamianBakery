...
const updatePage = async () => {
  ...
  const unhealthy = (fruit) => {
    return fruit.nutritions.sugar > 10;
  }

  const unhealthyFruit = fruitsArray.filter(unhealthy).map(function (fruit) { return '<li>' + fruit.name + '</li>'});

  console.log(unhealthyFruit);
  // TODO: Create a new HTML element to display your data 
  const newP = document.createElement("p");
  newP.innerHTML = "Fruit with more than 10 sugar";

  const newDiv = document.createElement("ul");
  newDiv.innerHTML = unhealthyFruit.join('');
  newDiv.display = "flex";
